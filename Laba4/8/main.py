...
french_speaking = data.where(pandas.Series(['French' in str(d) for d in data.languages])).name.dropna()
...
southern = data.where(pandas.Series([float(str(d).split(',')[0]) < 0 for d in data.latlng])).name.dropna()
...
print('\n\nCountries by areas')
for i, group in data.groupby([d for d in data.area]):
    print(str(i) + ': ')
    print_names(group.name)
    print()
...
lat.name = 'latitude'
lng.name = 'longitude'
for_export = pandas.concat([names, data[['capital', 'area', 'currency']], lat, lng], axis=1)
